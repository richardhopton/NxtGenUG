...
namespace NxtGenXAMLNugget
{
    public class Helper
    {
        ...
        {
            String fileName = @"D:\Nugget_A_FilmList.{0}";
            Helper.TestXmlSerialize(TestObject, fileName, output, prompt);
            Helper.TestXmlDeserialize(typeof(FilmCollection), fileName, output, prompt);
            Helper.TestXamlSerialize(TestObject, fileName, output, prompt);
